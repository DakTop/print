 ...
 * Time    12:53
 */
public class LabelModelSeven extends LabelModel {
    ...
    private LabelPoint shopName = mLabelPointFactory.createPointWithRatioXY(0.035f, 0.060f, 8,8);
    private LabelPoint shopProductName = mLabelPointFactory.createPointWithRatioXY(0.142f, 0.243f, 8,8);
    private LabelPoint barCode = mLabelPointFactory.createPointWithRatioXY(0.148f, 0.828f, -4,-10);
    private LabelPoint unit = mLabelPointFactory.createPointWithRatioXY(0.236f, 0.565f, -8 * 6, 6);
    
    private LabelPoint shopProductPrice = mLabelPointFactory.createPointWithRatioXY(0.557f, 0.734f, 0,-3*8);

    private List<LabelProductInfo> productInfoList;

    public LabelModelSeven(List<LabelProductInfo> productInfoList) {
        this.productInfoList = productInfoList;
    }

    //单行1号字体
    private LabelPoint shopProductNameSmallSingleLine = mLabelPointFactory.createPointWithRatioXY(0.177f, 0.248f, -2 * 8, 8);
    //单行2号字体
    private LabelPoint shopProductNameBigSingleLine = mLabelPointFactory.createPointWithRatioXY(0.180f, 0.213f, -2 * 8, 8);
    //字符超过16为 上下显示
    private LabelPoint productNameLineFirst = mLabelPointFactory.createPointWithRatioXY(0.177f, 0.199f, -2 * 8, 8);
    private LabelPoint productNameLineSecond = mLabelPointFactory.createPointWithRatioXY(0.177f, 0.278f, -2 * 8, 8);
    //
    ...
    private void addMyText(LabelCommand tsc, LabelProductInfo productInfo) {
        if (!TextUtils.isEmpty(productInfo.shopName)) {
            addChineseText(tsc, shopName, toSmallerString(productInfo.shopName, 20));
        }
        ...
        if (!TextUtils.isEmpty(productInfo.shopProductPrice)) {
            ...
            tsc.addText(shopProductPrice.x , shopProductPrice.y, LabelCommand.FONTTYPE.SIMPLIFIED_CHINESE, LabelCommand.ROTATION.ROTATION_0, LabelCommand.FONTMUL.MUL_2, LabelCommand.FONTMUL.MUL_2, "￥");
            tsc.addText(shopProductPrice.x + 48, shopProductPrice.y + priceDownValue, fontType, LabelCommand.ROTATION.ROTATION_0, fontMul, fontMul, shouJia);
        }
    }
