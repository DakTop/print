 ...
 * Time    12:53
 */
public class LabelModelTwelve extends LabelModel {
    ...
    private LabelPoint barCode = mLabelPointFactory.createPointWithRatioByFactorY(0.160f, 0.700f, -16);
    private LabelPoint priceBackground = mLabelPointFactory.createPointWithRatioByFactorY(0.150f, 0.500f, -8);
    private LabelPoint shopProductPrice = mLabelPointFactory.createPointWithRatioXY(0.200f, 0.460f, 8, 0);

    private List<LabelProductInfo> productInfoList;

    public LabelModelTwelve(List<LabelProductInfo> productInfoList) {
        this.productInfoList = productInfoList;
    }

    //单行1号字体
    private LabelPoint shopProductNameSmallSingleLine = mLabelPointFactory.createPointWithRatioXY(0.130f, 0.0800f, -2 * 8, 8);
    //单行2号字体
    private LabelPoint shopProductNameBigSingleLine = mLabelPointFactory.createPointWithRatioXY(0.130f, 0.0600f, -2 * 8, 8);
    //字符超过16为 上下显示
    private LabelPoint productNameLineFirst = mLabelPointFactory.createPointWithRatioXY(0.130f, 0.045f, -2 * 8, 8);
    private LabelPoint productNameLineSecond = mLabelPointFactory.createPointWithRatioXY(0.130f, 0.145f, -2 * 8, 8);
    private LabelPoint productNameLineThird = mLabelPointFactory.createPointWithRatioXY(0.130f, 0.245f, -2 * 8, 8);
    //
    private static final int BIG_FONT_SIZE_SINGLE_LINE_COUNT = 12;
    private static final int SMALL_FONT_SIZE_SINGLE_LINE_COUNT = 24;
    private static final int SMALL_FONT_SIZE_TWO_LINE_COUNT = 48;
    ...
    private void addMyText(LabelCommand tsc, LabelProductInfo productInfo) {
        ...
        if (!TextUtils.isEmpty(productInfo.shopProductPrice)) {
            ...
            tsc.addText(priceBackground.x, priceBackground.y, LabelCommand.FONTTYPE.SIMPLIFIED_CHINESE, LabelCommand.ROTATION.ROTATION_0, LabelCommand.FONTMUL.MUL_1, LabelCommand.FONTMUL.MUL_1, "￥          元" + danWei);
